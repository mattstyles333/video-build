...
if [ "${DOCSYNC_SKIP_LLM:-0}" = "1" ]; then
  ...
  if [ -n "$OC" ]; then
    ...
    for attempt in 1 2; do
      if timeout 900 "$OC" run --auto -m "${DOCSYNC_MODEL:-opencode-go/grok-4.5}" "$PROMPT"; then
        ok=1
        break
      fi
      log "opencode pass failed (attempt $attempt) — retrying"
      sleep 10
    done
    [ "$ok" = 1 ] || log "opencode pass failed after retries — committing synced content anyway"
  else
